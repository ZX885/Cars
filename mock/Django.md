Permissions - это разрешение на что нибудь
...
---Это тип атаки, при которой злоумышленник внедряет вредоносный код на веб-сайт. Этот код можно использовать для кражи конфиденциальной информации у пользователя или для перенаправления пользователя на вредоносный веб-сайт. Чтобы предотвратить этот тип атаки, нам необходимо убедиться, что весь пользовательский ввод правильно экранирован, прежде чем он будет отображен на веб-сайте. Мы можем сделать это, используя escape-фильтр в наших шаблонах или используя escape-функцию в наших представлениях.
```html
{{ user_input|escape }}
```
...
---Это тип атаки, при которой злоумышленник обманом заставляет пользователя отправить запрос на веб-сайт без его ведома. Это можно сделать, отправив пользователю ссылку, содержащую вредоносный запрос, или используя вредоносный скрипт для отправки запроса на сайт. Чтобы предотвратить атаки такого типа, нам необходимо убедиться, что все формы на нашем веб-сайте имеют токен CSRF. Мы можем сделать это, используя тег csrf_token в наших шаблонах или используя декоратор csrf_protect в наших представлениях.
```html
<form>
 {% csrf_token %}
</form>
...
RU: Сигналы используются для выполнения кода, когда в Django происходят определенные события. Например, мы можем использовать сигналы для выполнения кода при создании пользователя или при входе пользователя в систему. Мы также можем использовать сигналы для выполнения кода при сохранении или удалении модели. Сигналы полезны для выполнения кода, который не связан напрямую с представлением, например для отправки электронного письма при создании пользователя или обновления модели, когда пользователь входит в систему.
```python
import os
...
```
Then in views.py or in models.py
```python
...
and inside it we should create folders for each language that we want to use for example: ru, en, uz.
